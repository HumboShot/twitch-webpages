...
window.addEventListener('onWidgetLoad', function (obj) {
    ...
    const subName = nameElement.dataset.name || "";
    const finalCount = parseInt(countElement.dataset.count) || 1; // Default to 1 for new subs

    // Generate and display the AUTH_CODE derived from the subscriber's name.
    document.querySelector("#auth-hash").innerText = generateAuthHash(subName);
